...
function getOutcomeColor(label: string): { bg: string; text: string; border: string; bar: string } {
  ...
  if (normalized === 'YES') return { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-300', bar: 'bg-emerald-500' };
  if (normalized === 'NO') return { bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-300', bar: 'bg-red-500' };
  if (normalized === 'DRAW') return { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-300', bar: 'bg-amber-500' };
  // Pastel palette for custom outcomes
  const pastels = [
    { bg: 'bg-violet-50', text: 'text-violet-700', border: 'border-violet-300', bar: 'bg-violet-500' },
    { bg: 'bg-sky-50', text: 'text-sky-700', border: 'border-sky-300', bar: 'bg-sky-500' },
    { bg: 'bg-pink-50', text: 'text-pink-700', border: 'border-pink-300', bar: 'bg-pink-500' },
    { bg: 'bg-teal-50', text: 'text-teal-700', border: 'border-teal-300', bar: 'bg-teal-500' },
    { bg: 'bg-orange-50', text: 'text-orange-700', border: 'border-orange-300', bar: 'bg-orange-500' },
    { bg: 'bg-indigo-50', text: 'text-indigo-700', border: 'border-indigo-300', bar: 'bg-indigo-500' },
  ];
...
function getWagerStatusBadge(status: string) {
  switch (status) {
    case 'WON': return 'bg-emerald-100 text-emerald-800 ring-1 ring-emerald-300';
    case 'LOST': return 'bg-red-100 text-red-800 ring-1 ring-red-300';
    case 'ACTIVE': return 'bg-blue-100 text-blue-800 ring-1 ring-blue-300';
    case 'REFUNDED': return 'bg-amber-100 text-amber-800 ring-1 ring-amber-300';
...
function getBetStatusBadge(status: string) {
  switch (status) {
    case 'OPEN': return 'bg-green-50 text-green-700 ring-1 ring-green-200';
    case 'CLOSED': return 'bg-yellow-50 text-yellow-700 ring-1 ring-yellow-200';
    case 'RESOLVED': return 'bg-purple-50 text-purple-700 ring-1 ring-purple-200';
    case 'CANCELLED': return 'bg-red-50 text-red-700 ring-1 ring-red-200';
    case 'PAUSED': return 'bg-orange-50 text-orange-700 ring-1 ring-orange-200';
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8">
          <div className="flex items-center justify-between py-1">
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-blue-50 ring-1 ring-blue-100 flex items-center justify-center flex-shrink-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-100 flex items-center justify-center flex-shrink-0 group-hover:bg-blue-200 transition-colors">
                <svg className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              </div>
              <span className="hidden sm:inline-flex items-center gap-0.5 text-[10px] font-medium text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-full">
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg>
                Active
              </span>
            </div>
            <p className="text-xl sm:text-2xl font-bold text-gray-900 mb-0.5">${profile?.balance?.toLocaleString() || '0'}</p>
            <p className="text-[10px] sm:text-xs text-gray-400 font-medium">Current Balance</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-purple-100 flex items-center justify-center flex-shrink-0 group-hover:bg-purple-200 transition-colors">
              ...
              {(profile?.stats?.totalBets ?? profile?.totalBets ?? 0) > 0 && (
                <span className="hidden sm:inline-flex items-center gap-0.5 text-[10px] font-medium text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-full">
                  +{profile?.stats?.totalBets ?? profile?.totalBets ?? 0} placed
                </span>
              )}
            </div>
            <p className="text-xl sm:text-2xl font-bold text-gray-900 mb-0.5">{profile?.stats?.totalBets ?? profile?.totalBets ?? 0}</p>
            <p className="text-[10px] sm:text-xs text-gray-400 font-medium">Total Bets Placed</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-100 flex items-center justify-center flex-shrink-0 group-hover:bg-emerald-200 transition-colors">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-amber-100 flex items-center justify-center flex-shrink-0 group-hover:bg-amber-200 transition-colors">
              ...
              {(profile?.stats?.totalWinnings ?? 0) > 0 && (
                <span className="hidden sm:inline-flex items-center gap-0.5 text-[10px] font-medium text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-full">
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" /></svg>
                  Profit
                </span>
              )}
        ...
        {activeTab === 'MY_BETS' && (
          <>
            ...
            {myBetsFilter === 'HISTORY' && (
              <div className="mb-6 bg-white border border-gray-200 p-4 rounded-xl">
                ...
                {currentBetsList.map((bet: any) => {
                  ...
                  return myBetsViewMode === 'list' ? (
                    ...
                    <div
                      key={bet.wager?.id || bet.id}
                      onClick={() => handleOpenBetDetails(bet)}
                      className={`group bg-white rounded-xl border shadow-sm hover:shadow-md transition-all cursor-pointer overflow-hidden ${
                        isWon ? 'border-emerald-200 ring-1 ring-emerald-100' :
                        isLost ? 'border-red-200 ring-1 ring-red-100' :
                        'border-gray-200 hover:border-blue-200'
                      }`}
                    >
                      {/* Win/Loss accent bar */}
                      {myBetsFilter === 'COMPLETED' && (
                        <div className={`h-1 ${isWon ? 'bg-emerald-500' : isLost ? 'bg-red-500' : 'bg-amber-500'}`} />
                      )}

                      <div className="p-4 sm:p-5">
                        {/* Title + Badges */}
                        <div className="flex items-start justify-between gap-2 sm:gap-3 mb-2.5 sm:mb-3">
                          <h3 className="text-sm sm:text-base font-semibold text-gray-900 leading-snug line-clamp-2 group-hover:text-blue-600 transition-colors">
                            {bet.title}
                          </h3>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div>
                            ...
                            <p className="font-semibold text-gray-900">${bet.wager?.amount?.toLocaleString() || 0}</p>
                          </div>
